...
import { can, ROLE_LABELS, type AdminRole, type AdminSection } from "@/lib/auth/roles";
import { AdminStoreProvider } from "../_store";
import { ConfirmProvider } from "./confirm";
import { logout } from "../auth-actions";

// `section: null` = open to every signed-in role (the dashboard, scoped inside).
const nav: {
  href: string;
  label: string;
  ...
  { href: "/admin", label: "Dashboard", icon: LayoutDashboard, exact: true, section: null },
  { href: "/admin/berita", label: "Berita & Kegiatan", icon: Newspaper, section: "berita" },
  { href: "/admin/tata-kelola", label: "Tata Kelola", icon: Users, section: "tata-kelola" },
  { href: "/admin/lowongan", label: "Lowongan Kerja", icon: Briefcase, section: "lowongan" },
  { href: "/admin/lamaran", label: "Lamaran (CV)", icon: FileText, section: "lamaran" },
];
...
export default function PanelShell({
  ...
  const visibleNav = nav.filter((item) => !item.section || can(role, item.section));
    ...
    <div className="flex h-full flex-col">
      <div className="mx-4 flex items-center justify-between gap-2 border-b border-white/15 px-1 pb-7 pt-7">
        ...
        <span
          ...
          className="shrink-0 rounded-md bg-white/15 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-[0.1em] text-white/80"
        >
          {role === "master" ? "Admin" : ROLE_LABELS[role]}
        </span>
      </div>
